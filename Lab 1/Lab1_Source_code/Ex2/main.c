...
int main(void)
{
    ...
    while (1)
    {
        // Turn on RED LED (PA5) for 5 seconds
        ...
        // Turn off RED LED (PA5), turn on YELLOW LED (PA6) for 2 seconds
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);  // RED off
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_RESET);  // YELLOW on
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET);  // GREEN off
        HAL_Delay(2000);

        // Turn off RED (PA5) and YELLOW (PA6) LEDs, turn on GREEN LED (PA7) for 3 seconds
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);  // RED off
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_SET);  // YELLOW off
...
static void MX_GPIO_Init(void)
{
    ...
    // Initialize the YELLOW LED (PA6)
    GPIO_InitStruct.Pin = GPIO_PIN_6;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

    // Initialize the GREEN LED (PA7)
    GPIO_InitStruct.Pin = GPIO_PIN_7;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

    // Initially turn off all LEDs to avoid unintended behavior
