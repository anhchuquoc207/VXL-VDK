...
volatile uint8_t timer_multiplex_flag = 0;
volatile uint8_t timer_1s_flag = 0;
volatile uint8_t timer_dot_flag = 0;

static volatile int32_t t_multiplex = 0;
static volatile int32_t t_1s = 0;
static volatile int32_t t_dot = 0;

static inline int32_t clamp_min1(int32_t v) { return (v <= 0) ? 1 : v; }

static inline void setTimer_multiplex(uint32_t duration_ms) {
    int32_t ticks = (int32_t)MS_TO_TICKS(duration_ms);
    t_multiplex = clamp_min1(ticks);
    timer_multiplex_flag = 0;
}
static inline void setTimer_1s(uint32_t duration_ms) {
    int32_t ticks = (int32_t)MS_TO_TICKS(duration_ms);
    t_1s = clamp_min1(ticks);
    timer_1s_flag = 0;
}
static inline void setTimer_dot(uint32_t duration_ms) {
    int32_t ticks = (int32_t)MS_TO_TICKS(duration_ms);
    t_dot = clamp_min1(ticks);
    timer_dot_flag = 0;
}
static inline void timer_run(void) {
    if (t_multiplex > 0) { if (--t_multiplex == 0) timer_multiplex_flag = 1; }
    if (t_1s       > 0) { if (--t_1s       == 0) timer_1s_flag       = 1; }
    if (t_dot      > 0) { if (--t_dot      == 0) timer_dot_flag      = 1; }
}
...
int main(void)
{
  ...
  setTimer_multiplex(10);
  setTimer_1s(1000);
  setTimer_dot(500);

  while (1) {
    if (timer_multiplex_flag) {
      timer_multiplex_flag = 0;
...
static void MX_GPIO_Init(void)
{
  ...
  GPIO_InitStruct.Pin = LED2_Pin|LED1_Pin|EN0_Pin|EN1_Pin|EN2_Pin|EN3_Pin;
